...
def tokenize_sents(sentence_pairs: List[SentencePair], source_dict, target_dict) -> List[TokenizedSentencePair]:
    ...
    for pair in sentence_pairs:
        tokens = [[source_dict[eng], target_dict[zch]] for eng, zch in zip(pair.source, pair.target) if
                     eng in eng_vocab_words and zch in zch_vocab_words]

        if not len(tokens):
            continue
        parsed_pairs = np.array(tokens)
        tokenized.append(TokenizedSentencePair(parsed_pairs[:, 0], parsed_pairs[:, 1]))

...
from modules.preprocessing import LabeledAlignment
...
def compute_precision(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> Tuple[int, int]:
    ...
    for ref, pred in zip(reference, predicted):
        sign = set(pred)
        poss = set(ref.possible)
        sure = set(ref.sure)

        num += len((sure | poss) & sign)
        den += len(sign)
...
def compute_aer(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> float:
    ...
    """
    prc_num, prc_den = compute_precision(reference, predicted)
    rec_num, rec_den = compute_recall(reference, predicted)

    return 1 - (prc_num + rec_num) / (prc_den + rec_den)